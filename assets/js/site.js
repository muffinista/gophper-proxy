$(document).ready(function() {
  ...
  window.addEventListener('popstate', function(event) {
    ...
    if ( state.data ) {
      $("#gopher").html(state.data).fromGopher();

      // update our breadcrumb nav
      updateBreadcrumb(state.url);
    }
    else {
  ...
  var loadGopherUri = function(params) {
    ...
    $.ajax({
      ...
      //
        ...
        // URI as well
        history.pushState({ data: data.data, url: data.url }, data.url, data.url);
  ...
  if ( $("#gopher").html() !== "" ) {
    history.pushState({ data: $("#gopher").html(), url:"/" }, "/", "/");
    $("#gopher").fromGopher();
  }
});
